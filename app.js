...
function standortAufKartenZeichnen(position) {
    ...
    if (hauptkarte) {
        if (!mainLocationMarker) {
            ...
            mainLocationMarker.setLatLng(coordinates);
        }
    ...
    if (trackingKarte) {
        if (!trackingLocationMarker) {
            ...
            trackingLocationMarker.setLatLng(coordinates);
        }
...
function markerSetzen(typ, latitude, longitude) {
    trackingKarteInitialisieren();

    if (!trackingKarte || typeof L === "undefined") {
        return;
    }
    ...
    const oldMarker = isStart ? trackingStartMarker : trackingLandingMarker;
    ...
    if (isStart) {
        ...
        trackingKarte.setView(coordinates, 13);
    } else {
...
function zeichneTrackingTrack(track) {
    ...
    const route = (Array.isArray(track) ? track : [])
    ...
    if (trackingRouteLine) {
        trackingKarte.removeLayer(trackingRouteLine);
    }
    ...
    trackingRouteLine = L.polyline(route, {
        color: "#16a2ff",
        weight: 4,
        opacity: 0.92,
        lineCap: "round",
    ...
    const bounds = trackingRouteLine.getBounds();

    if (bounds.isValid()) {
        trackingKarte.fitBounds(bounds, {
            padding: [25, 25],
            maxZoom: 15
        });
    }
...
function trackingKarteZuruecksetzen() {
    ...
    [trackingStartMarker, trackingLandingMarker, trackingRouteLine].forEach(function (layer) {
        if (layer) {
...
function alleFluegeAufKarte() {
    ...
    if (!hauptkarte || !allFlightsLayer || typeof L === "undefined") {
        return;
    }
    ...
    fluege.forEach(function (flight) {
        const route = (Array.isArray(flight.track) ? flight.track : [])
            .filter(mapPointValid)
            .map(function (point) {
                return [Number(point.lat), Number(point.lng)];
            });
    ...
    if (allCoordinates.length > 0 && !letzterStandort) {
        hauptkarte.fitBounds(allCoordinates, {
            padding: [20, 20],
            maxZoom: 11
        });
    }
...
function flugAufKarte(flight) {
    ...
    if (!hauptkarte || !selectedFlightLayer || typeof L === "undefined") {
        return;
    }
    ...
    L.circleMarker(route[0], {
        ...
        .bindPopup(`Start: ${htmlSicher(flight.startOrt || "Unbekannt")}`);
    ...
    if (route.length > 1) {
        ...
        L.circleMarker(route[route.length - 1], {
            ...
            .addTo(selectedFlightLayer)
            .bindPopup(`Landung: ${htmlSicher(flight.landeOrt || "Unbekannt")}`);
    }

    const bounds = L.latLngBounds(route);

    if (bounds.isValid()) {
        hauptkarte.fitBounds(bounds, {
            padding: [30, 30],
            maxZoom: 15
        });
    }
...
function standortAnzeigeAktualisieren(position) {
    ...
    textSetzen("currentLatitude", formatKoordinate(latitude));
    textSetzen("currentLongitude", formatKoordinate(longitude));
    textSetzen(
        "currentAltitude",
    ...
    textSetzen(
        ...
        Number.isFinite(Number(speed))
            ? `${formatZahl(Math.max(0, Number(speed)) * 3.6, 1)} km/h`
            : "Nicht verfügbar"
    );
    textSetzen(
        "locationUpdatedAt",
        new Date(position.timestamp || Date.now()).toLocaleTimeString("de-AT")
    );
    textSetzen("locationStatus", "GPS-Position aktiv");
    textSetzen(
        "trackingCoordinates",
        `${formatKoordinate(latitude)}, ${formatKoordinate(longitude)}`
    );
    textSetzen(
        "trackingAccuracy",
        Number.isFinite(Number(accuracy))
            ? `GPS-Genauigkeit: ± ${Math.round(Number(accuracy))} m`
            : "GPS-Genauigkeit: --"
    );
}
...
function standortFehler(error) {
    ...
    const message = texts[error?.code] || "GPS-Position nicht verfügbar";

    textSetzen("locationStatus", message);
    textSetzen("trackingCoordinates", message);
}
...
function standortZentrieren(map) {
    if (!letzterStandort) {
        ...
        textSetzen("locationStatus", "GPS-Position wird ermittelt ...");
        return;
    }

    map?.setView(
        [letzterStandort.coords.latitude, letzterStandort.coords.longitude],
        14,
        { animate: true }
    );
}
...
function trackpunktHinzufuegen(position) {
    const point = {
        ...
        hoehe: Number.isFinite(Number(position.coords.altitude))
            ? Number(position.coords.altitude)
            : null,
        speed: Number.isFinite(Number(position.coords.speed))
            ? Math.max(0, Number(position.coords.speed))
            : null,
        genauigkeit: Number.isFinite(Number(position.coords.accuracy))
            ? Number(position.coords.accuracy)
            : null,
        zeit: new Date(position.timestamp || Date.now()).toISOString()
    };
    ...
    const lastPoint = trackpunkte[trackpunkte.length - 1];

    if (lastPoint && entfernung(lastPoint, point) < 0.003) {
        return;
    }
...
function statusSetzen(text, className = "") {
    ...
    status.className = `status-message ${className}`.trim();
}

function buttonStatusSetzen(startDisabled, stopDisabled, saveDisabled) {
    if (element("startButton")) {
        element("startButton").disabled = startDisabled;
    }

    if (element("stopButton")) {
        element("stopButton").disabled = stopDisabled;
    }

    if (element("saveButton")) {
        element("saveButton").disabled = saveDisabled;
    }
}
...
function geolocationFehlerText(error) {
    ...
    return messages[error?.code] || "Der Standort konnte nicht ermittelt werden.";
}

function flugStarten() {
    if (aktuellerFlug) {
        window.alert("Es ist bereits eine Fahrt aktiv.");
        return;
    }

    if (!navigator.geolocation) {
        window.alert("GPS wird von diesem Browser nicht unterstützt.");
        return;
    }

    const pilot = textBereinigen(element("pilot")?.value);
    const ballon = textBereinigen(element("ballon")?.value).toUpperCase();
    const ballontyp = textBereinigen(element("ballontyp")?.value);

    if (!pilot || !ballon) {
        window.alert("Bitte zuerst Pilot und Ballon-Kennzeichen unter Einstellungen eintragen.");
        seiteAnzeigen("einstellungen");
        return;
    }

    statusSetzen("Startposition wird ermittelt ...", "status-working");
    ...
    navigator.geolocation.getCurrentPosition(
        async function (position) {
            ...
            aktuellerFlug = {
                ...
                ballon,
                ballontyp,
                startOrt: await ortName(position.coords.latitude, position.coords.longitude),
            ...
            markerSetzen("start", position.coords.latitude, position.coords.longitude);
            ...
            statusSetzen("● Fahrt läuft", "status-running");
            buttonStatusSetzen(true, false, true);
        },

        function (error) {
            statusSetzen(`❌ ${geolocationFehlerText(error)}`, "status-error");
            buttonStatusSetzen(false, true, true);
        },
...
function flugBeenden() {
    if (!aktuellerFlug || flugWirdBeendet) {
        return;
    }

    flugWirdBeendet = true;

    statusSetzen("Landeposition wird ermittelt ...", "status-working");
    buttonStatusSetzen(true, true, true);
    ...
    navigator.geolocation.getCurrentPosition(
        async function (position) {
            ...
            aktuellerFlug.endezeit = new Date().toISOString();
            aktuellerFlug.landeOrt = await ortName(position.coords.latitude, position.coords.longitude);
            aktuellerFlug.landeLat = position.coords.latitude;
            aktuellerFlug.landeLng = position.coords.longitude;

            markerSetzen("landung", position.coords.latitude, position.coords.longitude);

            statusSetzen("✓ Fahrt beendet. Bitte jetzt speichern.", "status-running");
            buttonStatusSetzen(true, true, false);
        },
...
function flugSpeichern() {
    ...
    const start = new Date(aktuellerFlug.startzeit);
    const end = new Date(aktuellerFlug.endezeit);
    ...
    for (let index = 1; index < trackpunkte.length; index += 1) {
        distance += entfernung(trackpunkte[index - 1], trackpunkte[index]);
    }
    ...
    Object.assign(aktuellerFlug, {
        flugzeit: flightMinutes,
        strecke: Number(distance.toFixed(1)),
        track: [...trackpunkte],
        ...
        landungen: Math.max(1, Number.parseInt(element("landungen")?.value, 10) || 1),
        bemerkung: textBereinigen(element("bemerkung")?.value)
    });
    ...
    statusSetzen("✓ Fahrt erfolgreich gespeichert.", "status-running");
    buttonStatusSetzen(false, true, true);
...
function zeichneHoehenprofil(track) {
    if (typeof Chart === "undefined" || !element("heightChart")) {
        return;
    }
    ...
    hoehenChart = new Chart(element("heightChart"), {
        type: "line",
        data: {
...
function zeichneMonatsstatistik() {
    ...
    for (let offset = 11; offset >= 0; offset -= 1) {
        ...
        labels.push(
            monthDate.toLocaleDateString("de-AT", {
                month: "short",
                year: "2-digit"
            })
        );
    ...
    monatsChart = new Chart(element("monthlyChart"), {
        type: "bar",
        data: {
            labels,
            datasets: [
...
function summenBerechnen() {
    return {
        time: fluege.reduce((sum, flight) => sum + (Number(flight.flugzeit) || 0), 0),
        distance: fluege.reduce((sum, flight) => sum + (Number(flight.strecke) || 0), 0),
        landings: fluege.reduce((sum, flight) => sum + (Number(flight.landungen) || 0), 0)
    };
}
...
function zusammenfassungAktualisieren() {
    ...
    textSetzen("countFlights", fluege.length);
    textSetzen("countLandings", totals.landings);
    textSetzen("countMinutes", formatFlugzeit(totals.time));
    textSetzen("countKm", `${formatZahl(totals.distance, 1)} km`);
    ...
    const rows = [
        ["Gesamtfahrten", fluege.length],
        ["Gesamtlandungen", totals.landings],
        ["Flugzeit", formatFlugzeit(totals.time)],
        ["Strecke", `${formatZahl(totals.distance, 1)} km`],
        ["Durchschnittsdauer", formatFlugzeit(fluege.length ? totals.time / fluege.length : 0)],
        ["Letzte Fahrt", lastFlight ? formatDatum(lastFlight.startzeit || lastFlight.datum) : "-"]
    ];
...
function rekordeAktualisieren() {
    ...
    const records = [
        ["Schnellste Fahrt", `${formatZahl(maxValue("maxSpeed"), 1)} km/h`],
        ["Höchste Höhe", `${formatZahl(maxValue("maxHoehe"))} m`],
        ["Längste Strecke", `${formatZahl(maxValue("strecke"), 1)} km`],
        ["Längste Dauer", formatFlugzeit(maxValue("flugzeit"))]
    ];
...
function flugLoeschen(index) {
    ...
    const removed = fluege.splice(index, 1)[0];

    if (!fluegeSpeichern()) {
        fluege.splice(index, 0, removed);
        return;
    }
...
function sichereGanzzahl(value, fallback, minimum = 0, maximum = Number.MAX_SAFE_INTEGER) {
...
function monitorAktualisieren() {
    const target = element("aktivitaetsmonitorInhalt");
    ...
    cutoff.setMonth(cutoff.getMonth() - settings.zeitraumMonate);

    const relevantFlights = fluege.filter(function (flight) {
        const date = new Date(flight.startzeit || flight.datum);
    ...
    const flightMinutes = relevantFlights.reduce(
        (sum, flight) => sum + (Number(flight.flugzeit) || 0),
        0
    );
    ...
    const landings = relevantFlights.reduce(
        (sum, flight) => sum + (Number(flight.landungen) || 0),
        0
    );

    function progressHtml(label, value, targetValue, displayValue = value, displayTarget = targetValue) {
    ...
    const formattedFlightHours = formatZahl(flightHours, 1);
    const formattedRequiredHours = formatZahl(settings.erforderlicheStunden, 1);

    const fulfilled =
        flightHours >= settings.erforderlicheStunden &&
    ...
    textSetzen("monitorPeriodLabel", `${settings.zeitraumMonate} MONATE`);

    if (relevantFlights.length > 0) {
        textSetzen(
            "nextExit",
            `${formattedFlightHours} Flugstunden, ${landings} Landung(en) und ${relevantFlights.length} Fahrt(en) im eingestellten Zeitraum.`
        );
    } else {
        textSetzen("nextExit", "Keine relevante Fahrt im Zeitraum.");
    }
}
...
function monitorFormularFuellen() {
    ...
    if (element("monitorZeitraum")) {
        element("monitorZeitraum").value = settings.zeitraumMonate;
    }

    if (element("monitorSollStunden")) {
        element("monitorSollStunden").value = settings.erforderlicheStunden;
    }

    if (element("monitorSollLandungen")) {
        element("monitorSollLandungen").value = settings.erforderlicheLandungen;
    }
}
...
function monitorSpeichern() {
    ...
    try {
        localStorage.setItem(MONITOR_STORAGE_KEY, JSON.stringify(settings));
        monitorFormularFuellen();
        monitorAktualisieren();
        textSetzen("monitorMeldung", "✓ Anforderungen wurden gespeichert.");
    } catch (error) {
        console.error(error);
        textSetzen("monitorMeldung", "❌ Anforderungen konnten nicht gespeichert werden.");
    }
}
...
function monitorZuruecksetzen() {
    try {
        ...
        monitorFormularFuellen();
        monitorAktualisieren();
        textSetzen("monitorMeldung", "✓ Einstellungen wurden zurückgesetzt.");
    } catch (error) {
        console.error(error);
        textSetzen("monitorMeldung", "❌ Einstellungen konnten nicht zurückgesetzt werden.");
    }
}
...
async function wetterAnzeigen(latitude, longitude) {
    const rating = element("weatherRating");
    ...
    try {
        ...
        textSetzen(
            ...
            Number.isFinite(temperature)
                ? `🌡 Temperatur: ${formatZahl(temperature, 1)} °C`
                : "🌡 Temperatur: --"
        );
        ...
        textSetzen(
            ...
            Number.isFinite(direction)
                ? `🧭 Windrichtung: ${Math.round(direction)}° (${windrichtungText(direction)})`
                : "🧭 Windrichtung: --"
        );

        textSetzen("sunrise", "🌅 Sonnenaufgang: " + isoUhrzeit(daily.sunrise?.[0]));
        textSetzen("sunset", "🌇 Sonnenuntergang: " + isoUhrzeit(daily.sunset?.[0]));

        let assessment = "🟢 Geringe Bodenwindgeschwindigkeit.";
        let className = "weather-good";

        if (!Number.isFinite(wind)) {
            assessment = "Wetterbewertung nicht verfügbar.";
            className = "weather-caution";
        } else if (wind > 20) {
            assessment = "🔴 Hohe Bodenwindgeschwindigkeit.";
            className = "weather-danger";
        } else if (wind > 10) {
            assessment = "🟡 Erhöhte Bodenwindgeschwindigkeit.";
            className = "weather-caution";
        }

        if (rating) {
            rating.textContent =
                `${assessment} Luftfeuchtigkeit ${
            ...
            rating.className = `weather-rating ${className}`;
        }
    } catch (error) {
        console.error(error);

        if (rating) {
            rating.textContent = "❌ Wetterdaten konnten nicht geladen werden.";
            rating.className = "weather-rating weather-danger";
        }
    }
}
...
function ladeWetter() {
    ...
    rating.textContent = "Wetterdaten werden geladen ...";
    rating.className = "weather-rating";

    if (letzterStandort) {
        wetterAnzeigen(letzterStandort.coords.latitude, letzterStandort.coords.longitude);
        return;
    }

    if (!navigator.geolocation) {
        rating.textContent = "GPS wird nicht unterstützt.";
        rating.className = "weather-rating weather-danger";
        return;
    }
    ...
    navigator.geolocation.getCurrentPosition(
        function (position) {
            ...
            wetterAnzeigen(position.coords.latitude, position.coords.longitude);
        },
        function () {
            rating.textContent = "Standortfreigabe ist für Wetterdaten erforderlich.";
            rating.className = "weather-rating weather-caution";
        },
        {
            maximumAge: 300000,
            timeout: 15000
...
function importDialogOeffnen() {
    ...
    textSetzen("importFileInformation", "Noch keine Datei ausgewählt.");
    ...
    const message = element("importMessage");
    ...
    if (element("executeImportButton")) {
        element("executeImportButton").disabled = true;
    }
...
function importZahl(value, fallback = 0) {
...
function csvZeileAufteilen(line, separator) {
    const values = [];
    let currentValue = "";
    let quoted = false;
...
function csvEinlesen(text) {
    ...
    if (lines.length < 2) {
        throw new Error("Die CSV-Datei enthält keine Fahrten.");
    }
    ...
    const headers = csvZeileAufteilen(lines.shift(), separator).map(function (header) {
        return header
            .toLowerCase()
            .replace(/\s+/g, "")
            .replace(/ä/g, "ae")
    ...
    return lines.map(function (line) {
        const values = csvZeileAufteilen(line, separator);
...
function importFlugNormalisieren(rawFlight, index) {
    ...
    const startTime = importDatum(
        getValue("startzeit", "startZeit", "datum", "date")
    );
    ...
    let track = getValue("track", "trackpunkte", "route");
    ...
    return {
        ...
        flugzeit: Math.max(0, importZahl(getValue("flugzeit", "dauer"))),
        strecke: Math.max(0, importZahl(getValue("strecke", "kilometer"))),
        avgSpeed: Math.max(0, importZahl(getValue("avgSpeed", "avgspeed"))),
        maxSpeed: Math.max(0, importZahl(getValue("maxSpeed", "maxspeed"))),
        maxHoehe: importZahl(getValue("maxHoehe", "maxhoehe")),
        minHoehe: importZahl(getValue("minHoehe", "minhoehe")),
        avgHoehe: importZahl(getValue("avgHoehe", "avghoehe")),
        landungen: Math.max(1, Math.round(importZahl(getValue("landungen"), 1))),
        bemerkung: textBereinigen(getValue("bemerkung", "notiz")),
...
async function importDateiAuswaehlen(event) {
    ...
    try {
        ...
        vorbereiteteImportFluege = rawFlights
            .map(importFlugNormalisieren)
            .filter(Boolean);

        if (vorbereiteteImportFluege.length === 0) {
        ...
        if (preview) {
            preview.innerHTML = `
                <strong>✓ Datei erfolgreich geprüft</strong>
                <p>${vorbereiteteImportFluege.length} Fahrt(en) und ${landingCount} Landung(en) gefunden.</p>
            `;
            preview.hidden = false;
        }

        if (element("executeImportButton")) {
            element("executeImportButton").disabled = false;
        }

        const message = element("importMessage");

        if (message) {
            message.textContent = "Die Datei kann importiert werden.";
            message.className = "import-message import-message-success";
        }
    } catch (error) {
        console.error(error);
        ...
        if (message) {
            message.textContent = `❌ ${error.message || "Importdatei ist ungültig."}`;
            message.className = "import-message import-message-error";
        }
    }
}
...
function duplikatSchluessel(flight) {
    return [
        textBereinigen(flight.startzeit).toLowerCase(),
        textBereinigen(flight.ballon).toUpperCase(),
        textBereinigen(flight.pilot).toLowerCase()
    ].join("|");
}
...
function importAusfuehren() {
    ...
    if (mode === "replace") {
        if (!window.confirm("Alle vorhandenen Fahrten werden ersetzt. Fortfahren?")) {
            return;
        }

        fluege = [...vorbereiteteImportFluege];
    } else {
        const existingKeys = new Set(fluege.map(duplikatSchluessel));

        vorbereiteteImportFluege.forEach(function (flight) {
            const key = duplikatSchluessel(flight);
...
function kopfbereichAktualisieren() {
    const pilot = textBereinigen(element("pilot")?.value);
    const ballon = textBereinigen(element("ballon")?.value);
    const ballontyp = textBereinigen(element("ballontyp")?.value);

    textSetzen("pilotHeader", pilot || "Pilot");
    textSetzen("trackingPilotDisplay", pilot || "Nicht festgelegt");
    textSetzen("trackingBallonDisplay", ballon || "Nicht festgelegt");
    textSetzen("trackingTypeDisplay", ballontyp || "Nicht festgelegt");

    const maintenance = element("maintenanceInput")?.value;

    textSetzen(
        "maintenanceDate",
        maintenance
            ? new Date(`${maintenance}T12:00:00`).toLocaleDateString("de-AT")
            : "Nicht festgelegt"
    );
}
...
function eventHinzufuegen(id, eventName, handler) {
    element(id)?.addEventListener(eventName, handler);
}

function appInitialisieren() {
    const masterData = stammdatenLaden();

    ["pilot", "ballon", "ballontyp"].forEach(function (id) {
        if (element(id)) {
            element(id).value = masterData[id] || "";
        }
    });

    if (element("maintenanceInput")) {
        element("maintenanceInput").value = masterData.maintenance || "";
    }
    ...
    document.querySelectorAll("[data-page-link]").forEach(function (link) {
        link.addEventListener("click", function (event) {
            ...
            seiteAnzeigen(link.dataset.pageLink);
        });
    });
    ...
    eventHinzufuegen("saveButton", "click", flugSpeichern);

    ["pilot", "ballon", "ballontyp", "maintenanceInput"].forEach(function (id) {
        eventHinzufuegen(id, "change", stammdatenSpeichern);
    });

    eventHinzufuegen("monitorSpeichern", "click", monitorSpeichern);
    eventHinzufuegen("monitorZuruecksetzen", "click", monitorZuruecksetzen);
    eventHinzufuegen("weatherReload", "click", ladeWetter);

    eventHinzufuegen("showCurrentLocationButton", "click", function () {
        hauptkarteInitialisieren();
        standortZentrieren(hauptkarte);
    });

    eventHinzufuegen("showAllFlightsButton", "click", alleFluegeAufKarte);
    eventHinzufuegen("exportButton", "click", backupExportieren);
    eventHinzufuegen("openImportButton", "click", importDialogOeffnen);
    eventHinzufuegen("closeImportButton", "click", importDialogSchliessen);
    eventHinzufuegen("cancelImportButton", "click", importDialogSchliessen);
    eventHinzufuegen("importInput", "change", importDateiAuswaehlen);
    eventHinzufuegen("executeImportButton", "click", importAusfuehren);
    eventHinzufuegen("exportCsvButton", "click", exportCsv);

    const searchInput = element("flightSearch");
    if (searchInput) {
        searchInput.addEventListener("input", flugbuchAktualisieren);
    }

    const filterPilot = element("flightFilterPilot");
    if (filterPilot) {
        filterPilot.addEventListener("change", flugbuchAktualisieren);
    }

    const filterBalloon = element("flightFilterBalloon");
    if (filterBalloon) {
        filterBalloon.addEventListener("change", flugbuchAktualisieren);
    }

    const filterYear = element("flightFilterYear");
    if (filterYear) {
        filterYear.addEventListener("change", flugbuchAktualisieren);
    }
    ...
    eventHinzufuegen("importDialog", "click", function (event) {
        if (event.target === element("importDialog")) {
            importDialogSchliessen();
        }
    });

    eventHinzufuegen("flugliste", "click", function (event) {
        ...
        if (button.dataset.flightDelete !== undefined) {
            flugLoeschen(Number(button.dataset.flightDelete));
            return;
        }

        if (button.dataset.flightMap !== undefined) {
            const flight = fluege[Number(button.dataset.flightMap)];
    ...
    document.addEventListener("keydown", function (event) {
        if (event.key === "Escape" && !element("importDialog")?.hidden) {
            importDialogSchliessen();
        }
    });
    ...
    offlineBanner.textContent = "Offline-Modus aktiv – Daten bleiben lokal verfügbar.";
